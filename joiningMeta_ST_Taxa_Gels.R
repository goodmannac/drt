...
#splicing and rejoining with dashes tree, treatment, and tipID
...
#joining gel MGV
gels <- read.csv("~/Desktop/Bogar Lab/DRTSTgels.csv")
gels2 = gels %>%unite("tip.ID", 5:7, sep = "-")

md.w.Loc = full_join(metadata, taxa4, by = "tip.ID")
md.w.Loc1 = full_join(md.w.Loc, gels2, by = "tip.ID")

write.csv(md.w.Loc1, "~/Desktop/Bogar Lab/Meta Data with ST taxa and gels.csv")
